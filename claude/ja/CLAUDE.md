...
- `planner` — 計画文書をディスクへ書き込みます。既定モデルは `sdd-worker setup`
  で明示的に設定します (ユーザー設定に `planner.model` として保存され、
  エージェント定義の `model:` frontmatter に書き込まれます。既定値は `opus` で、
  暗黙のフォールバックではありません。組織にFableのアクセス権があれば setup で
  `fable` を選べます)。ユーザーは委譲ごとに上書きもできます。
  Agent ツールの `model` パラメーターを渡してください (例: "planはopusで" →
  `model: "opus"`)。これはその 1 回の委譲に限り、設定済みの frontmatter より
  優先されます。返すのはファイルパスとタスク一覧だけです。単純でない設計には
  必ず使用します。会話コンテキストを持たずに開始するため、explorer の調査結果と
  ユーザー要件を常に明示的に渡してください。自分で計画を作成したり書き写したり
  してはいけません。
